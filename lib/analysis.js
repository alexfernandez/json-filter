...
exports.Analyzer = function(pass)
{
	...
	self.analyze = function(data)
	{
		...
		analyzePackage(data);
		microprofiler.measureFrom(start, 'analyzePackage', 100000);
		if (self.response)
		{
			return;
		}
	...
	function analyzeFullMessage(buffer)
	{
		...
		microprofiler.measureFrom(start, 'readStart', 100000);
		pos += 2;
		while (buffer[pos] != 13 || buffer[pos + 1] != 10)
		{
			var headerStart = pos;
		...
		pos += 2;
		microprofiler.measureFrom(start, 'readHeaders', 100000);
	...
	function analyzeBody(buffer, pos)
	{
		...
		if (self.headers['content-type'] == 'application/json')
		{
			try
			{
				...
				microprofiler.measureFrom(start, 'parseJson', 100000);
			}
			catch(error)
			{
