...
exports.start = function(options, callback)
{
	...
	var server = net.createServer(function(connection)
	{
		...
		var socket = openConnection(options, function(error)
		{
			...
			{
				connection.end();
				return log.error('Could not open connection to backend: %s', error);
			}
			socket.setNoDelay();
			...
			connection.on('data', function(data)
			{
				...
				{
					var sent = socket.write(requestAnalyzer.resend);
					requestAnalyzer.init();
					microprofiler.measureFrom(start, sent ? 'request-resent' : 'request-buffered', 100000);
				}
			...
			socket.on('data', function(data)
			{
				...
				{
					var sent = connection.write(responseAnalyzer.resend);
					responseAnalyzer.init();
					microprofiler.measureFrom(start, sent ? 'response-resent' : 'response-buffered', 100000);
				}
