...
exports.start = function(options, callback)
{
	var server = net.createServer(function(connection)
	{
		...
		connection.on('data', function(data)
		{
			...
			if (!options.delay)
			{
				...
				microprofiler.measureFrom(start, 'writeNoContent', 100000);
				return;
			}
			setTimeout(function()
			{
				connection.write(HTTP_NO_RESPONSE);
				microprofiler.measureFrom(start, 'writeDelayed', 100000);
			}, options.delay);
		});
		connection.on('error', function(error)
		{
			log.debug('Error on connection: %s', error);
			connection.end();
		});
	...
	{
		log.debug('Server error: %s', error);
	});
